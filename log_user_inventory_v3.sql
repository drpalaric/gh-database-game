...
create or replace function golfheroes_v3.user_inventory_transactions_fn (userid integer, itemid integer, item_amount integer, item_level integer default 0)
    returns void as $$
DECLARE
...
BEGIN
    ...
    IF EXISTS (select from golfheroes_v3.user_inventory_log where user_id = userid) THEN
        ...
        ELSIF EXISTS (select from golfheroes_v3.items where id = itemid) THEN
            ...
            (user_id,   item_id,    previous_item_level,    new_item_level,     previous_amount,    new_amount,     transaction_kind,   transaction_value) values 
            (userid,    itemid,     0,                      0,                  0,                  0,              'CREATION',         0), 
            (userid,    itemid,     0,                      item_level,         0,                  item_amount,    'CREDIT',           item_amount);
...
create trigger update_user_items_trigger after insert on golfheroes_v3.user_inventory_log
    for each row 
