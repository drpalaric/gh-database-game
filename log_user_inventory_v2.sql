select golfheroes.user_inventory_transactions_fn2 (243, 116, 1, 1);
select * from golfheroes.user_inventory_log where user_id = 243 and item_id = 116;
select * from golfheroes.user_inventory where item_id = 116 and user_id = 243;

create or replace function golfheroes.user_inventory_transactions_fn (userid integer, itemid integer, item_amount integer, item_source text, item_level integer default 0)
    returns void as $$
DECLARE
...
BEGIN
    ...
    IF EXISTS (select from golfheroes.user_inventory_log where user_id = userid) THEN

        IF EXISTS (select from golfheroes.user_inventory_log where user_id = userid and item_id = itemid) THEN

            -- if the user and the item already exists in the log, update the log with the new values for that item

                IF EXISTS (select from golfheroes.items where id = itemid and is_unique is false)  THEN

                    insert into golfheroes.user_inventory_log (user_id, item_id, previous_item_level, new_item_level, previous_amount, new_amount, transaction_kind, transaction_value, source)
                        with current_user_item as (
                            select 
                                ...
                                previous_amount,
                                new_amount
                            from golfheroes.user_inventory_log
                ...
                ELSIF EXISTS (select from golfheroes.items where id = itemid and is_unique is true) and item_amount <= 1 THEN 

                    select into user_item_amount amount from golfheroes.user_inventory where user_id = userid and item_id = itemid;
    ...
    ELSE 
        ...
        with default_vals AS (
            ...
            select 
                ...
                'CREATION' as source
            from golfheroes.default_inventory RETURNING *
...
create or replace function golfheroes.user_update_items_fn() 
returns trigger language plpgsql as $set_user_items$
begin

    IF EXISTS (select from golfheroes.user_inventory where user_id = NEW.user_id) THEN

        IF EXISTS (select from golfheroes.user_inventory where user_id = NEW.user_id and item_id = NEW.item_id) THEN

            update golfheroes.user_inventory 
                set 
        ...
        ELSE 
            ...
            insert into golfheroes.user_inventory (user_id, item_id, amount, level)
...
create trigger update_user_items_trigger_v3 after insert on golfheroes.user_inventory_log
    for each row 
    execute procedure golfheroes.user_update_items_fn();


----
insert into golfheroes.user_inventory_log (user_id, item_id, previous_item_level, new_item_level, previous_amount, new_amount, transaction_kind, transaction_value)
...
from golfheroes.user_inventory;

insert into golfheroes.user_inventory         
